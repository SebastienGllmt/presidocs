...
test("deriveFigureOccurrences: lead-up staging + sticky within a sub-chapter", () => {
  ...
  expect(deriveFigureOccurrences(marks, 0, 10000, 10000)).toEqual([
    ...
    // covering the whole occurrence — today's behavior (proposal 49 back-compat).
...
// --- per-step schedule within an occurrence (proposal 49) -------------------
...
// mirroring `stagedFigureAt`'s step walk (proposal 48 §7.2).
...
// --- held-frame index math (proposal 49 §3.2) -------------------------------
