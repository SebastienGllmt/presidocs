...
//
// This file is a single-post script: the stages below are orchestrated by
// `main()` behind `if (import.meta.main)`, so importing this module is
// side-effect free (the pure stage helpers are unit-tested in
// generate.test.ts). Batch mode over every narrated post is delegated to
// generate-all.ts, which spawns this same script once per post.
...
import { ttsProviders, type PlsLexicon, type SegmentContext, type TtsProvider } from "./tts-providers.ts";
import { extractNarration, extractPlsBlocks, splitChapter, type NarrationChapter } from "./narration.ts";
import { wrapWithCache, computeTextHash, computeCacheKey, type CachedTtsProvider, type TtsCacheIdentity } from "./tts-cache.ts";
...
// Returns the kept chapters (or the input unchanged when no allowlist is set).
export function applyChapterFilter(
  chapters: NarrationChapter[],
  chapterAllowlist: Set<string> | null,
...
// TTS provider.
async function assemblePlsSources(
  htmlPath: string,
  projectRoot: string,
  inlinePlsBlocks: string[],
): Promise<{ plsSources: PlsSource[]; localPlsSources: PlsSource[] }> {
  ...
  return { plsSources, localPlsSources };
}

const lexemeBodyRegex = /<lexicon\b[^>]*>([\s\S]*?)<\/lexicon\s*>/;
export function mergeLexicons(sources: PlsSource[]): PlsLexicon {
...
export function resolveForcedTexts(chapters: NarrationChapter[], forceMarks: Set<string>): Set<string> {
...
// Preflight: sanity-check the provider's output format against the pipeline,
// then confirm every required binary (TTS + pipeline) is on PATH.
function preflight(tts: TtsProvider, pipeline: AudioPipeline): void {
...
type ChapterArtifact = {
  ...
  localMarks: {
    ...
    words?: CachedWord[];
    // The mark's stage/control pointer (methodology.md → "Staging a figure from narration"). Absent (undefined) =
    // "leave the stage unchanged"; "none"/"" = an explicit clear; otherwise a
    // `<figure id>`. Carried verbatim onto the manifest mark.
    figure?: string;
    // The mark's per-step slideshow pointer (methodology.md → "Staging a figure from narration"). Absent (undefined) =
...
async function renderChapters(opts: {
  chapters: NarrationChapter[];
  pipeline: AudioPipeline;
  tts: TtsProvider;
  cachedAligner: CachedAligner | null;
  ttsIdentity: TtsCacheIdentity;
  mock: boolean;
  rate: number;
  segmentGapMs: Milliseconds;
  segmentGap: Uint8Array | null;
  outDir: string;
}): Promise<{
  artifacts: ChapterArtifact[];
  silentMarks: { mark: string; chapter: string; durationMs: Milliseconds }[];
}> {
  const { chapters, pipeline, tts, cachedAligner, ttsIdentity, mock, rate, segmentGapMs, segmentGap, outDir } = opts;
    ...
    for (const [i, seg] of segments.entries()) {
      ...
      if (seg.markName) {
        localMarks.push({
          ...
          // Same treatment for the per-step pointer (methodology.md → "Staging a figure from narration").
...
export function buildManifestParts(
  artifacts: ChapterArtifact[],
  chapters: NarrationChapter[],
  segmentGapMs: Milliseconds,
): { manifestChapters: ManifestChapter[]; manifestMarks: ManifestMark[]; duration: Milliseconds } {
  ...
  for (const [i, a] of artifacts.entries()) {
    ...
    for (const m of a.localMarks) {
      ...
      manifestMarks.push({
        ...
        // serialized manifest stays byte-identical (methodology.md → "Audio caching").
        ...(m.figure !== undefined ? { figure: m.figure } : {}),
        // Same conditional spread for the per-step pointer (methodology.md → "Audio caching"): an
        // unset `step` omits the key, so un-annotated marks stay byte-identical
        // and only annotated marks change — keeping the cache invariant.
        ...(m.step !== undefined ? { step: m.step } : {}),
      });
    }
    offset = end;
  }
  return { manifestChapters, manifestMarks, duration: offset };
}
...
async function writeAudio(
  outDir: string,
  pipeline: AudioPipeline,
  fullDelivered: Uint8Array,
): Promise<{ fullName: string; audioDigest: string }> {
  ...
  return { fullName, audioDigest };
}

// The manifest-name hash covers only the narration-bearing fields {audio,
// audioDigest, duration, chapters, marks} — NOT generatedAt/slug/audioBytes/
// provenance — so an unchanged regenerate keeps the same name and stays
// cache-warm. Locked by generate.test.ts (D4).
export function hashedManifestName(manifest: {
  audio: string;
  audioDigest: string;
  duration: Milliseconds;
...
async function main(): Promise<void> {
  ...
  const { flags, positional } = parseGenerateArgs(argv);
  ...
  //
  // Both collections come out of the post via ./narration.ts (HTMLRewriter,
  // Bun built-in, lol-html under the hood — a proper streaming HTML parser;
  // regex on HTML is unsound). `<script>` content is RAWTEXT so its inner
  // `<` doesn't confuse the walker and entities aren't decoded, so the
  // narration and PLS payloads come out byte-identical to what the author
  // wrote. Two block types:
  //   - `text/narration` — the spoken-script for one chapter (extractNarration,
  //     which also reports `data-narration="none"` opt-out)
  ...
  //     bootstrap)
  const { disabled, chapters: rawChapters } = extractNarration(html);
  const inlinePlsBlocks = extractPlsBlocks(html);
  ...
  normalizeChapterParents(rawChapters);

  const chapters = applyChapterFilter(rawChapters, chapterAllowlist, chapterFilter);
  ...
  const { plsSources, localPlsSources } = await assemblePlsSources(htmlPath, projectRoot, inlinePlsBlocks);
  ...
  const forcedTexts = resolveForcedTexts(chapters, forceMarks);
  ...
  const { artifacts, silentMarks } = await renderChapters({
    chapters,
    pipeline,
    tts,
    cachedAligner,
  ...
  const { manifestChapters, manifestMarks, duration: offset } = buildManifestParts(artifacts, chapters, segmentGapMs);
  ...
  const { fullName, audioDigest } = await writeAudio(outDir, pipeline, fullDelivered);
  ...
  const manifestHash = hashedManifestName(manifest);
