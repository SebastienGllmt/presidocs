...
// Asset/codegen serving (the generated-audio byte server + the on-the-fly
// Markdown / figure-source codegen twins) was split out of this file — see
// server/devServerServing.ts. This file keeps the entry factory + the route
// table (registrations) that wire those servers to the resolved paths.
import { serveFigureSource, serveFromDir, serveMarkdown } from "./devServerServing.ts";
...
export async function createDevServer(opts: DevServerOptions) {
  ...
  return {
    ...
    async fetch(req: Bun.BunRequest) {
      // The `/posts/<slug>.md` and figure-source codegen twins live in
      // server/devServerServing.ts; the `paths` they used to close over is
      // threaded in here (the same paths the route table above is wired to).
      const md = await serveMarkdown(req, paths);
      if (md) return withSecurityHeaders(md);
      const fig = await serveFigureSource(req, paths);
