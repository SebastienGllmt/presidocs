...
2. **Fetch the open comments** as a Web Annotation collection:
   ...
   - `motivation` — usually `"commenting"`. `"editing"` marks a **suggestion** (propose-an-edit): the first body is a `TextualBody` with `purpose: "editing"` / `format: "text/plain"` whose `value` is the proposed replacement for the anchored `exact` text (empty `value` = delete the range). Apply it near-verbatim — locate `exact`, swap in the proposed text — unless it would break an editing rule (then PARTIAL/NOTE-ONLY, per the rules). The proposed text may carry literal `<em>…</em>` / `<strong>…</strong>` markup (the in-place editor is WYSIWYG); apply those as real HTML tags in the source, not as escaped text. A structured suggestion is *easier* than prose: the change is spelled out. Any `commenting` bodies after it are notes explaining the intent.
