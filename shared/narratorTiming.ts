...
export type FigureStateMark = Timed & {
  ...
  // The per-step slideshow pointer (methodology.md → "Staging a figure from narration"), orthogonal to `figure`:
 ...
 *
 * Driven by the `marks[].figure` stage pointer (methodology.md → "Staging a figure from narration"): a `figure` value
...
export interface StagedFigure {
   ...
   * the stage is empty.
   *
   * The narration driver (methodology.md → "Live figure driving") advances a staged journey by
  ...
  /**
   * The active step label driving the staged figure (methodology.md → "Live figure driving"), or null for
   ...
   * clears it explicitly. The narration driver (methodology.md → "Live figure driving") reads this: a
 ...
 * driving it (methodology.md → "Live figure driving"). Same sub-chapter-bounded walk as
...
export function stagedFigureAt<T extends FigureStateMark>(
  ...
  for (const m of marks) {
    ...
    // case, methodology.md → "Staging a figure from narration") records that step rather than having the figure
 ...
 * target, honoring the forward-only contract (methodology.md → "Animated figures" → "The FigureJourney contract"): advance only
