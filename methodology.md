...
- **Cards column** is a `position: absolute` frame in the right gutter. Each card is anchored to its highlight (text), graphic root (figure), or article element (narration) via CSS [Anchor Positioning][CSSAnchorPos1]: `renderAll` stamps `anchor-name` on the anchor and `position-anchor` on the card, and the rule `top: anchor(top, 0px)` aligns the card's top with the anchor in the column's coordinate space. The browser re-evaluates `anchor()` on every scroll frame, so cards stay vertically aligned with no JS scroll listener. Stale text threads (no highlight) fall back to anchoring on the first segment block. The `position-try-fallbacks: --cmt-pin-to-column-top` rule catches the case where the anchor is scrolled clear of the viewport. Cards do **not** push later cards down to avoid overlap — anchor positioning aligns each card independently, and we accept the visual stacking when two anchors land within ~card-height of each other; a `--cmt-stack-offset` JS pass can be added later if a real post needs it.
