...
- **<1100px (popover mode):** the column's permanent surfaces (identity header, version banner, history panel, stacked cards) are hidden. Each card carries `popover="auto"` (the native [Popover API][Popover]), so the platform renders it in the **top layer** when shown — escaping every clipping ancestor and any z-index ladder the narration drawer might be inside. Placement is **anchored to the tapped element** via [CSS Anchor Positioning][CSSAnchorPos1]: the same `anchor-name` stamped on the highlight for the desktop column also drives the mobile popover's `position-area: block-end span-all` (centered below the anchor; the `:popover-open` rule first resets `inset: auto` so the desktop card's `top: anchor(top)` doesn't leak in and offset it); `position-try-fallbacks: flip-block, --cmt-bottom-sheet` flips to above the anchor if "below" overflows, then falls through to a viewport-pinned bottom-sheet (reserving room above the player via `inset-block-end: calc(var(--narrate-dock-height, 0px) + 24px)`) when both anchored placements overflow. No JS measures rects — the engine resolves all of it natively, and re-evaluates on scroll and on visual-viewport change (so the popover follows its anchor when the iOS soft keyboard contracts the viewport, instead of needing the pre-2026 "compute placement before focus()" workaround). Light-dismiss (any pointer outside) and ESC are platform behaviors; a `toggle` event listener on each card syncs `activeCardId` when the platform closes the popover. (One deliberate interception: ESC on an *empty draft* discards it outright rather than just light-dismissing the popover — the composer's keydown handles that case, per the Drafts note above.) Tapping a highlight or graphic indicator calls `showPopover()` on its card; tapping the *same* anchor a second time calls `hidePopover()` (the platform's `auto` popover doesn't toggle on re-tap of an arbitrary invoker, so we do that one bit explicitly); tapping a *different* highlight just calls `showPopover()` on the new one — `popover="auto"` enforces only-one-at-a-time and closes the prior popover for us. Drafts created via the action-bar Comment pill are immediately surfaced. Multi-thread anchors still resolve to a single popover (the first thread); a stacked-popover or swipe-between flow can land later without further architectural changes. The popover element is rebuilt by `renderAll`, so after each render we call `showPopover()` on the new card if `activeCardId` was still set.
...
Sessions are stored entirely in a single `HttpOnly` cookie holding a standard **JWT** (RFC 7519), HS256-signed via [`jose`](https://github.com/panva/jose) — a real `<header>.<payload>.<signature>` JWS compact serialization, so jwt.io / the `jose` CLI / any JWT debugger reads it, and `jose` owns the constant-time verify plus the algorithm allowlist (`{ algorithms: ["HS256"] }`, the standard `alg:none`/confusion defense). `iat`/`exp` are seconds since epoch per the spec. TTL is 400 days, which is the practical max — Chrome (since v104), Firefox, and Safari all clamp any cookie `Max-Age` beyond that to 400 days. Sliding-window refresh (re-mint on each authenticated request to keep active users logged in indefinitely) is a follow-up. `jose` runs on Web Crypto (`globalThis.crypto.subtle`), so `verifySessionToken` — and the `getSessionFromRequest` → `whoami` callers it feeds — became `async`; the migration also drops the `node:crypto` HMAC the bespoke format relied on. The claims are trusted *after* verify: `getSessionFromRequest` returns the decoded payload as the `Session` shape without re-checking its fields. A zod schema for the claim shape would additionally catch a malformed-but-*validly-signed* payload, but the token is self-issued (HS256 over the per-deployment `SESSION_SECRET`), so only our own bug could mint a misshapen one — pinning the claim shape is a deferred hardening to revisit with any auth change, not a live gap (and the only auth boundary zod doesn't already cover; the OAuth callback parsing and `safeReturnTo` are semantic guards, not shape).
...
- `client/commentsDom.ts` — block walker, hash-stability `normalizeText`, hide-all FAB localStorage. The full CommentSystem class isn't tested directly — it needs Automerge boot, identity fetch, polling controllers, and a CRDT store, all of which the same-file `commentsStore.test.ts` already exercises at the data layer without a DOM.
...
**Real-browser harness (live).** A Playwright-driven e2e tier lives in `presidocs/e2e/`, driven by the `playwright` *library* (not `@playwright/test`) from inside `bun test` — one runner, two surfaces. Files are named `*.e2e.ts` so the default `bun test` glob (which matches `.test.`/`.spec.`) never touches them; they run on demand via `bun run test:e2e`, which keeps the unit run Chrome-free. `e2e/harness.ts` launches the **system Chrome** (no ~150 MB binary download) and boots a content-repo dev server on an ephemeral port (`PRESIDOCS_E2E_BLOG` points at the blog; the engine has no posts of its own). What's covered today: the cheapest-highest-value real-browser checks — real CSS layout (a heading's `getBoundingClientRect()` is non-zero, which happy-dom can't produce) and an accessibility-tree guard (the comments column resolves as a `complementary` landmark named "Comments" and is never a `feed`, unassertable in happy-dom because it builds no a11y tree). Also live: **comment-card positioning** (`e2e/commentPositioning.e2e.ts`, Chromium + Firefox + WebKit) — it mints a dev session, drives the real selection → comment → submit flow (real CRDT + anchoring + upload), and asserts cards track their highlights, drafts anchor while composing (no scroll-to-top), below-fold cards stay put (don't vanish with their anchor), overlapping cards cascade instead of stacking, the header rail stays pinned in the gutter, and the mobile popover anchors below its highlight. The session-minting + UI-driven seeding it introduced is the reusable "comment fixture." Also live: **per-post accessibility** (`e2e/axe.e2e.ts`) — runs axe-core over the landing page and every post (the rendered WCAG A/AA bars a static pass can't see — contrast, JS-generated figure ARIA, Label-in-Name), excluding the vendored Shikwasa player; it ratchets two deferred rules (contrast, Label-in-Name — proposal 29 §2.6–2.7). Staged next (in [proposal 22](../proposals/22-real-browser-e2e-remaining.md)): the comment-card *accessibility-tree* snapshot (article/no-posinset/`aria-pressed`/`<details>`) and the Service-Worker lifecycle. The SW tier is the highest-value one and the reason the harness exists now (the PWA + SW foundation has shipped — [Offline / PWA](#offline--pwa-clientswjs-clientswregisterts-sharedinjectpwaheadts)), but it needs a `dev:edge` (built + `wrangler dev`) target rather than the fast dev server, because `swRegister.ts` deliberately doesn't register the SW under plain `bun run dev` (`__BUN_DEV__` gate). Until that lands it stays a manual `dev:edge` smoke (DevTools → Application → Service Workers shows "activated and is running"; a code change cleanly transitions vN → vN+1 with `clients.claim()` taking over the next navigation). Posture: Chromium-only; Safari/Firefox stay manual-quarterly via `scripts/release-check.md`. No CI lane is wired yet — `test:e2e` is on-demand, so there's no browser-flake debt until the SW tier or a real regression justifies it.
...
- `generated/<slug>/manifest.<hash>.json` + `generated/<slug>/*.mp3` → `dist/generated/<slug>/`
- `node_modules/@automerge/automerge/dist/automerge.wasm` → `dist/assets/automerge.wasm`

Build-internal files under `generated/` are deliberately excluded: the `.tts-cache/` buckets, the dev-only `.comments-dev/` fs-adapter blobs, the per-slug `cache-keys.json` GC index, and dotfiles (notably macOS `._*` AppleDouble sidecars, which would otherwise match the `*.mp3` glob).
