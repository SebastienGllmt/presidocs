...
- **Model load dominates.** Loading the ~1.7B transformer costs seconds; doing it per segment would swamp every build. So `moss` runs a **long-lived worker** (`generate/moss_worker.py`): the TS provider spawns it once, the model loads once, and thereafter each segment is one request over a line-delimited JSON protocol on stdin/stdout (the worker writes a WAV to a caller-chosen temp path; stdout carries only protocol JSON, all library/progress chatter goes to stderr). The worker is spawned **lazily on the first cache miss**, so a fully-cached rebuild — or any `--mock` run — never even starts Python.
- **Voice cloning is per-call, not a one-time step.** MOSS's API re-supplies the reference clip at generation time (`build_user_message(text=…, reference=[clip])`); there's no separate "train a speaker embedding once." The expensive thing we amortize by keeping the worker alive is the *model load*, not the cloning. The reference clip is passed as `--voice=<path.wav>` (reusing the provider's `voice` field, which already feeds the cache key, so two reference clips cache separately). MOSS has no PLS and no words/min rate — both are warned-and-ignored like `say` does for PLS.
