...
- `templates/content-repo/` — the canonical starter a new blog copies: thin `index.ts`/`worker.ts`/`wrangler.toml`, `package.json` (with the `link:` dep + per-blog scripts), a commented-example landing, and the `process-comments` skill. It ships **content-empty** (`bun run new-post` scaffolds the first post); the e2e fixture's sample post + figure live in `e2e/fixture-content/`, not here. `personal-blog` is a real instance of this template
...
Code: `client/comments.ts` (orchestrator) + `client/comments/*.ts` (selection bar, block index, cards, drafts, version banner, unresolved nav, figure triggers, mobile menu, suggestion mode + preview).

Google-Docs-style threads anchored to selections in the article body, selections in the spoken-script drawer, or whole graphics. Every thread (and every in-progress draft) renders as its own card in a right-side margin column that scrolls with the article. A thread can also carry a machine-readable propose-an-edit payload — see [Suggestions](#suggestions-propose-an-edit).
...
The in-memory shape is already the spec model; `authoring/annotationExport.ts` is the pure transform that turns a merged snapshot into spec-valid JSON-LD — each thread an `Annotation` (`motivation: "commenting"`, or `"editing"` for a [suggestion](#suggestions-propose-an-edit)), each reply a `TextualBody` (`text/markdown`), wrapped in an `AnnotationCollection`. Non-WA data becomes `x-blog:*` extensions; **`authorEmail` is dropped** (author-eyes-only data has no place in a portable export). IRIs are `urn:blog:<slug>:thread:<id>` — deliberately not `urn:uuid:`, because the ids aren't RFC-4122 UUIDs and claiming otherwise is a lie a strict consumer could reject.
...
- **Selection → action bar.** "Comment" and "Suggest edit" pills appear above any selection in a commentable root; clicking creates a draft card, scrolls to it, focuses its textarea. (Suggest edit prefills the selection as the proposed text — see [Suggestions](#suggestions-propose-an-edit).)
...
- **Highlight colour** is soft blue, deliberately not yellow — narration already paints the active sentence yellow/orange, and a sentence that's both read-aloud and commented must stay unambiguous. Overlapping threads compose to darker blue, reading as "denser commentary." Suggestion ranges tint green (a class on the same highlight span, so overlaps still compose) so a propose-an-edit reads apart from a plain comment.
...
Code: `client/comments/suggestMode.ts` (in-place edit session), `blockEdit.ts`/`emphasis.ts`/`suggestionDiff.ts` (pure diff/serialize surfaces), `suggestionPreview.ts` (author preview lens); the payload type lives in `commentsStore.ts`, export in `authoring/annotationExport.ts`.

A reader edits a piece of the article and the change becomes a thread the author can accept — the Google-Docs *gesture*, but deliberately not the Google-Docs *model*. There, a suggestion is a live insert/delete op inside a collaboratively-rebased document; here the published article is immutable per version and **git is the only write path to content — the [`process-comments`](#ai-assisted-authoring) loop is the accept button** — so a suggestion is just a comment whose payload is machine-readable (`original → proposed`) instead of prose. That reframe dissolves the hard problems by construction: no live range rebasing, no rich-text CRDT, no in-doc accept toggle. (Web Annotation even has the vocabulary ready: `motivation: "editing"`.)

- **Data model.** A thread's optional `suggestion` payload (`proposed` text + author identity) is written in the `addThread` change and **immutable after create** — so no concurrent-update case exists and no merge logic is needed. The original text is *not* stored twice: the target's `TextQuoteSelector.exact` is the original (self-contained even after orphaning); `proposed: ""` means delete the range. Identity rides the payload because threads carry none (normally inferred from the first reply, which a note-less suggestion doesn't have). Seed-safe: the field lives *inside* a thread — not a top-level doc-shape change, so the [schema-evolution freeze](#why-flat--fine-grained-updates) is not violated.
- **Two entry paths, one draft shape.** The "Suggest edit" pill (desktop action bar + mobile menu; login-gated exactly like Comment) opens a composer with the selection prefilled as editable proposed text and a live diff — zero DOM-mutation risk, and how mobile gets the feature (no contenteditable on mobile, ever). **In-place mode** (desktop-only rail toggle, not mounted without a session) makes a clicked article block `contenteditable`; committing produces exactly a pill-path suggestion, so card/diff/submit/export are all one path.
- **Capture against the original — the one subtle invariant.** The anchor must describe the *published* document (that's what the block hashes were computed from and what `process-comments` locates via `exact`), but the editing gesture mutates the DOM. So: snapshot the block on session start, diff on commit (minimal replaced window via common prefix/suffix trim; surrogate pairs respected; a pure insertion widens to the adjacent word so the range stays highlightable), then **swap the pristine snapshot back in — a suggestion never changes what any reader sees.** Edits are block-scoped on purpose (`beforeinput` blocks paragraph splits/drops), which amputates the split/merge anchor nightmares; structural rewrites stay prose comments.
- **Emphasis is the only rich text.** Ctrl/Cmd+B/I render real bold/italic in the editor (WYSIWYG — literal tags in the text was rejected in use); on commit the DOM serializes to text carrying only `<em>`/`<strong>` (every other wrapper collapses to inner text), the diff runs on that serialized form so a formatting-only change still registers, the window snaps outward to whole tags (**don't char-diff across tag swaps naively**: `<em>` vs `<strong>` share `<`/`>`, and cutting mid-token ships unbalanced markup in the proposal), and the window ends map back to *plain* offsets — anchor/quote/hash stay plain text; only `proposed` carries tags. The inverse parser treats *only* `<em>`/`<strong>` as markup, never `innerHTML`, so reader-supplied proposals can't inject. Paste is forced plain.
- **Commit semantics: Enter posts, clicking away drafts, Esc reverts.** Enter is the deliberate "done" (the saved card opens with the reply box focused — a note is an optional reply, not a pre-submit field); everything non-deliberate (blur, mode toggle-off, clicking another block) keeps a recoverable draft. The pill path always drafts — there, reviewing before submit is the point.
- **Rendering + main-thread guards.** Card diffs are word-granular via jsdiff (`diffWordsWithSpace` only — the sole adopted dependency, display-only; the anchor window is always the hand-rolled trim), debounced per keystroke, and above ~4k combined chars degrade to whole-string strike/insert (Myers-family diffs go ~O(n·d) and would lock the thread). **In-place mode must hold the citation generator suppressed for the whole session** (`setCommentBarActive(true)` held, released only on mode-off): releasing it per selection re-enables text-fragment generation on every `selectionchange`, which freezes hard on blocks fragmented by existing highlights — this was a real multi-second hang, misattributed to highlights at first. Editing a block *with* its highlights in place is fine; don't strip them for the session (tried — it made other cards jump).
- **Export + authoring loop.** A suggestion exports as `motivation: "editing"` with the proposal as its first body (`purpose: "editing"`, `text/plain`, `creator` from the payload); note replies follow as ordinary `commenting` bodies. **The zero-reply "nothing to say" guard excepts suggestions everywhere it exists** — the exporter *and* the offline loader — because a note-less suggestion's diff *is* its content. `process-comments` applies proposals near-verbatim (locating via `exact`), downgrading to PARTIAL/NOTE-ONLY only when an editing rule objects; proposed text may carry `<em>`/`<strong>` to apply as real HTML. Staleness needs nothing new and comes out *right*: applying suggestion A changes the block, so suggestion B on the same block orphans-and-flags (true — B was written against text that no longer exists) while A resolves via the normal loop.
- **Preview lens (author-only, any build).** A per-paragraph gutter eye-toggle flips a block to a display-only view with its suggestions applied (emphasis rendered, changes green-tinted). The canonical layer is never touched — stale detection compares stored hashes against the *index-time* block hash, not a live re-hash, so swapping displayed text can't disturb it — and the block is **read-only while previewed**: both commenting and in-place editing are suppressed, because the shown text isn't the anchor basis. Includes drafts (previewing before posting is the point); overlapping suggestions apply right-to-left as an approximation — the real merge is `process-comments` in git. Readers never get an applied view: the reading surface stays pristine, and "applied" is a deliberate lens, not the default.
- **Rejected:** mirroring article text into the comments CRDT for live suggestion spans (copies the post into every reader doc, drifts from the git-built HTML each publish, and buys merge semantics git already owns); adopting a track-changes editor (all require owning the document representation); an in-browser Accept that edits the site (breaches the dumb-server rule and the git-only write path); carrying the diff as a formatted first *reply* instead of a structured field (dual source of truth; cards would special-case hiding it); Google's merge-adjacent-edits-after-the-fact (requires re-entering a block seeded with your own open suggestion, i.e. relaxing immutable-after-create to owner-may-edit-own-open — safe but a deliberate data-model change, unbuilt).
- **Excluded from v1:** an accept/decline `outcome` on the [author-resolution](#author-resolution) envelope (plain Resolve covers it; the field fits the existing single-writer JSON with no server change, when wanted); a live per-keystroke side card during in-place editing (commit-on-Enter/blur avoids re-rendering under the caret); suggestion targets on narration/graphics; formatting beyond `<em>`/`<strong>`. Known ceiling: a paragraph whose *visible text* contains a literal `<em>`/`<strong>` sequence confuses the serialize/offset mapping.
...
Authoring leans on stable ids (staging marks, "rewrite paragraph `zswap-body`" prompts), but ids live in source, not on the rendered page. This module floats a small `#id` label in the left gutter beside every `<figure id>` and `<p id>` — click copies the bare id. **Author-only, any build**: gated on the server-authoritative `isAuthor` flag (a DOM authorship check can't work in prod where the meta tag is stripped) — reading a block's id off a deployed post is as useful as off localhost, and a non-author reader gets nothing. Idempotent. The [suggestion preview lens](#suggestions-propose-an-edit) shares this gutter (its toggle sits in the lane nearest the text; both affordances are absolutely positioned, so nothing shifts the reading measure).
...
**The self-contained fixture.** `e2e/harness.ts` defaults to a fixture *materialized into a scratch dir outside the repo* (`~/.cache/presidocs-e2e/<hash-of-engine-root>/`, override `PRESIDOCS_E2E_FIXTURE_ROOT`) by `ensureFixtureBlog()`: it copies the tracked `templates/content-repo` tree (via `git ls-files`, so never the residue), layers the committed `e2e/fixture-content/` overlay on top, then adds the per-machine setup products (an absolute `engine` symlink + `bun link`/`install` + a generated `SESSION_SECRET` written to both `.env` and `.dev.vars`, which must agree). Because the build runs in that scratch copy, a full `bun run test:e2e*` mutates nothing tracked — `git status` stays clean by construction. The whole e2e suite runs from a bare engine checkout, no private content repo. `PRESIDOCS_E2E_BLOG` retargets it at a real blog (built in place, as documented), and each content repo carries its own `test:e2e*` scripts, so the author's one-command flow still drives the real blog. Tests stay content-agnostic to serve both targets: single-post tiers drive the alphabetically-first deployable post; the figure gates drive every post whose *source markup* loads a figure module — detected from markup rather than the runtime registry, so a bundling/registration regression fails the gate loudly instead of vacuously skipping. The fixture's `hello.html` is a real post (commentable length, a conformant FigureJourney figure, the full client-script set, manifest + icons so the SW shell doesn't 404) and its overlay carries committed espeak narration audio (`e2e/fixture-content/blog/generated/`), so the audio-gated tiers — prodAudioSmoke and the subscribe browser tier — actually RUN on it, in CI too. The suite still carries no TTS/MOSS dependency *at test time*: the audio is bytes on disk, not synthesized during the run. CI (`.github/workflows/ci.yml`) runs two jobs against the fixture: `test` (typecheck + unit + the Chromium e2e loop) and `worker-tiers` (SW + CSP + links + the prod audio smoke sharing one build via `PRESIDOCS_E2E_SKIP_BUILD`, plus the private-blog tier `test:e2e:private`, which materializes its own private fixture — the public template + the private overlay). The `test:e2e:wrangler` script chains that same build-once sequence locally.
...
Everything above makes posts findable; a private blog (`BLOG_PRIVATE=1` — see the single starter's "Private blog" README section and the commented private landing variant in `templates/content-repo/index.html`) is the deliberate inversion: **the URL is the secret.** Every post filename carries an unguessable `--<token>` suffix (16 base64url chars ≈ 96 bits from `bun run new-post`; audit floor 64 bits), and everything derived from the slug — page, `.md` twin, audio, share card, `/post-version`, comment threads — inherits the secrecy because the slug is in the path. Entropy is calibrated to online guessing against the Worker, deliberately not to an offline adversary. **Renaming the file rotates the key** — leak recovery is rename, rebuild, redeploy.
