...
The greys `#777`–`#aaa` and the marginal accents (`#1f8a4c`/`#2f7d4d` green, `#2f7bb0` blue, `#8a7bbf`/`#7a6bb0` purple, `#b06a2f`/`#a83` amber) collapsed onto these. **Migration result (`contrastReport.ts`): 178 → 17 failing text nodes**; the 17 left are non-figure deliberate-dimming patterns deferred for a separate decision (the logged-out comments card — slated for a fundamental redesign; the `.redact` "🕶 hidden" placeholder; the narrator sub-chapter segments). **Every figure text node passes.** A recurring figure anti-pattern surfaced and was fixed: dimming an inactive step/node via container `opacity` (0.5–0.55) dragged its label below 4.5:1 — floored at ~0.78 with a dark base, or the opacity dropped where typography already de-emphasised the text.
...
- **Detect / triage (text)** — **`bun run e2e/contrastReport.ts [slug]`** runs axe's `color-contrast` over every page and prints each failing element's fg/bg/measured-ratio/required-ratio, **grouped by colour pair, worst-first**, with a PASS/FAIL trailer. This is the "what do I fix, and when do I pass" view (axe's gate only prints counts). Run it after any colour edit, and on a new figure, before trusting the eye.
- **Gate: text contrast (SC 1.4.3)** — `e2e/axe.e2e.ts` runs axe-core over the landing + every post. As of 2026-06 `color-contrast` is a **HARD GATE** (no longer ratcheted): any text below 4.5:1 (3:1 large) anywhere fails the suite. **Three** surfaces are exempted per-node via `CONTRAST_EXEMPT_SELECTORS` — `.cmt-identity-loggedout` (login card, dimmed-until-engaged, pending a fundamental redesign), `.redact` (redaction chips — faintness is the signal), `.ch-seg[data-sub]` (subordinate-by-dim sub-chapter pills). These are a **fixed, non-extensible roster of deliberate design decisions**, stripped per-node so every *other* rule still applies to them and any contrast failure *elsewhere* still hard-fails (proven by a negative test). Do not grow the list to silence a real failure — fix the colour (`contrastReport.ts` shows what).
